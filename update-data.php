...

{
     ...
     */
    private
    function downloadUrlToFile($url, $file, $updateTime)
    {
    ...
    function run($config, $update = 0, $upateOnly = '')
    {
        log_info("STARTING.. ");
        ...
        foreach($repos AS $repo) {
            ...
            if ((!$upateOnly) || ($upateOnly && ($upateOnly == $repoName))) {
                sleep(1);
                $this->downloadUrlToFile("https://api.github.com/repos/$githubOrg/$repoName/issues?state=all" . $apiCred, "json/repos/" . $repoName . "_issues.json", $update);
            }
            ...
            if ($update && ((!$upateOnly) || ($upateOnly && ($upateOnly == $repoName)))) {
                $this->downloadUrlToFile('https://raw.githubusercontent.com/' . $githubOrg . '/' . $repoName . '/master/README.md', "json/repos/" . $repoName . "_readme.md", $update);
            }
        ...
        file_put_contents("json/repos_meta.json", json_encode($allRepos, JSON_PRETTY_PRINT));
